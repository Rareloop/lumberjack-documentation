...
Lumberjack supports PSR-15/7 Middleware. Middleware can be added via the [Router](routing) to individual routes or route groups or via [WordPress Controllers](wordpress-controllers).
...
You can also apply Middleware on a Controller class too, either for use with the [Router](routing) or as a [WordPress Controller](wordpress-controllers). In order to do this your Controller must extend the `App\Http\Controllers\Controller` base class.
